...
class Transaksiinap extends CI_Controller {
   ...
   #   		die("<script>alert('Silahkan Login Admin');window.location='../dashboard'</script>");
    #  	}	
     ...
     #	{
       ...
       $data['data_obat']=$this->db->query('select * from data_obat where stok between 1 and 10');
        $data['data_obat']=$data['data_obat']->num_rows();
       $this->load->view('admin/transaksi/v_transaksi_rawat_inap',$data);
      # }
     #else {
      #   		die("<script>alert('Silahkan Login Admin');window.location='../dashboard'</script>");
       #  	}	
      ...
      public function cetak() {
        ...
        $visite=$this->input->post('visite');
        $kebersihan=$this->input->post('kebersihan');
        $konsultasi=$this->input->post('konsultasi');
        $total_harga=$rowinap->biaya+$bhp+$row1->biaya+$rowkamar->biaya+$dpjp+$visite+$kebersihan+$konsultasi;
...
$pdf->SetX(40);
        ...
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
        $pdf->Cell(40,10,'BHP: Rp. '.number_format($bhp));
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
        $pdf->Cell(40,10,'Obat:');
        //Table
        ...
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
        $pdf->Cell(40,10,'Visite            : Rp. '.number_format($visite));
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
        $pdf->Cell(40,10,'Kebersihan  : Rp. '.number_format($kebersihan));
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
        $pdf->Cell(40,10,'Konsultasi    : Rp. '.number_format($konsultasi));
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(5);
